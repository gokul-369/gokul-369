...
def get_user():
    ...
    """
    data = github_graphql(query, {"login": USER_NAME})
    return data["user"]
...
def get_contributions():
    ...
    """
    data = github_graphql(query, {"login": USER_NAME, "to": now})
...
def update_svg(filename, stats):
    ...
    replace_text(root, "age_data", stats["age"])
    replace_text(root, "commit_data", stats["commits"])
    replace_text(root, "star_data", stats["stars"])
    replace_text(root, "repo_data", stats["repos"])
    replace_text(root, "follower_data", stats["followers"])
